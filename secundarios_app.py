...
# ===================== SESSION STATE =====================
# Variáveis de estado mantidas para persistência dos inputs
...
with col_nome:
    # Usando key para persistência do valor
...
with col_assessor:
    # Usando key e default para persistência do valor
...
# O bloco de Salvar Simulação foi removido daqui
...
if not st.session_state.papeis:
    ...
    # Não usamos st.stop() aqui para não interromper a geração do PDF se a lista estiver vazia
    # a validação do PDF cuidará disso.
else:
    ...
    for papel in st.session_state.papeis:
        ...
        # Acessa a data de aplicação do session state
    ...
    if not resultados_calculados:
        ...
        # st.stop() deve ser usado aqui para evitar que o código que depende dos resultados
        # calculados tente rodar com listas vazias
        st.stop()
    ...
    # Funções de geração de PDF e gráfico permanecem inalteradas, mas foram condensadas
    # aqui para não poluir o código principal, e garantindo que usem as session_state keys.
